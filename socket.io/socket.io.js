// copy from https://github.com/flowtype/flow-typed/blob/master/definitions/npm/socket.io_v1.x.x/flow_v0.47.x-/socket.io_v1.x.x.js
...
declare interface SocketIO$Server {
  ...
  */
  use(
    fn: (socket: SocketIO$Socket, fn: (err?: any) => void) => any): SocketIO$Namespace,
...
declare type SocketIO$Namespace = {
  ...
  */
  use(
    fn: (socket: SocketIO$Socket, fn: (err?: any) => void) => void): SocketIO$Namespace,
 ...
 */
declare interface SocketIO$Emitter {
  /**
   ...
   * @return This Emitter
   */
  on(event: string, fn: Function): SocketIO$Emitter;

  /**
   * @see on( event, fn )
   */
  addEventListener(event: string, fn: Function): SocketIO$Emitter;
   ...
   * @return This Emitter
   */
  once(event: string, fn: Function): SocketIO$Emitter;
   ...
   * @return This Emitter
   */
  off(event: string, fn?: Function): SocketIO$Emitter;

  /**
   * @see off( event, fn )
   */
  removeListener(event: string, fn?: Function): SocketIO$Emitter;

  /**
   * @see off( event, fn )
   */
  removeEventListener(event: string, fn?: Function): SocketIO$Emitter;

  /**
   * Removes all event listeners on this object
   * @return This Emitter
   */
  removeAllListeners(): SocketIO$Emitter;
   ...
   * @return Emitter
   */
  emit(event: string, ...args: any[]): SocketIO$Emitter;
...
different return)
*/
declare interface SocketIO$Socket extends SocketIO$Emitter {
